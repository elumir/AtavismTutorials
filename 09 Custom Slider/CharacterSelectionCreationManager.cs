...
namespace Atavism
{
    ...
    public class CharacterSelectionCreationManager : MonoBehaviour
    {
        ...
        public OldUI.Button scarbutton;
        public OldUI.Button tattoobutton;
      ...
      //  public Texture cursorOverride;
        ...
        // Camera fields
        // EEE New Camera Variables - testing plastic
        ...
        // EEE UI Builder
        public IconRadioGroup m_gendersContainer;
        public VisualElement m_SectionHeadSliders;
        public VisualElement m_SectionTabsContainer; // tabs-container
        public IconTab m_IconTabHead; // holds the head tab because it requires a zoom in        

        public ColorPopup m_ColorPopup;
        public ColorField m_BodyColorField;
        public ColorField m_StubbleColorField;
                        ...
                        if (modularCharacterSlider.slider)
                        {
                            switch (modularCharacterSlider.dna)
                            {
                                ...
                                case "Eyebrow":
                                    ...
                                    break;
                                /* case "Scar":
                                     modularCharacterSlider.slider.onValueChanged.AddListener(id => { Switch((int) id); }); eyebrowModels.Count
                                     break;*/
                                case "Hands":
                                    modularCharacterSlider.slider.minValue = 0;
        ...
        void ResetModel()
        {
            ...
            if (modularCustomizationManager)
            {
                // EEE - New UI Builder Reset
                // Set Default colours. First for Model, then for UI
                SwitchSkinColortoColor(modularCustomizationManager.defaultSkinColor); 
                m_BodyColorField.SetValueWithoutNotify(modularCustomizationManager.defaultSkinColor);

                SwitchStubbleColortoColor(modularCustomizationManager.defaultStubbleColor);
                m_StubbleColorField.SetValueWithoutNotify(modularCustomizationManager.defaultStubbleColor);

                InitModularCharacterSliders(modularCustomizationManager);
        ...
        public void SwitchStubbleColortoColor(Color color) // EEE - Changed from Color32
        {
            ModularCustomizationManager modularCustomizationManager = character.GetComponent<ModularCustomizationManager>();
            if (modularCustomizationManager == null)
            {
      ...
      /*  public AtavismRaceData GetRaceDataByName(string raceName)
        ...
        public void UIInitModularCharacterComponents()
        {
            ...
            // Initiate Tabs
            m_SectionTabsContainer = root.Q<VisualElement>("tabs-container");
            for (i = 0; i < m_SectionTabsContainer.childCount; ++i)
            {
                VisualElement element = m_SectionTabsContainer[i];
                if (element is IconTab tabButton)
                {
                    tabButton.OnSelect += UITabClicked;
                }
            }
            // m_IconTabHead = root.Q<IconTab>("tabThree"); 

            // Initialize Gender Buttons
            m_gendersContainer = root.Q<IconRadioGroup>("gendersContainer");
            for (i = 0; i < m_gendersContainer.childCount; ++i)
            {
                VisualElement element = m_gendersContainer[i];
                if (element is IconRadio radioButton)
                {
                    radioButton.OnSelect += UIGenderClicked;
                }
            }

            // Initiate sliders in this Head section
            m_SectionHeadSliders = root.Q<VisualElement>("sectionHeadSliders"); 
            for (i = 0; i < m_SectionHeadSliders.childCount; ++i)
            {
                VisualElement element = m_SectionHeadSliders[i];
                if (element is ModCharSlider charSlider)
                {
                   charSlider.RegisterCallback<ChangeEvent<int>,ModularPart>(UIModularCharacterSetSlider, charSlider.modularPart);
                }
            }


            // Initialize colour buttons
            m_ColorPopup = root.Q<ColorPopup>("color-popup");
            m_BodyColorField = root.Q<ColorField>("body-color");
            m_StubbleColorField = root.Q<ColorField>("stubble-color");

            // Check if I can loop through child elements to do the below
            m_BodyColorField.ColorPopup = m_ColorPopup;
            m_StubbleColorField.ColorPopup = m_ColorPopup;
        ...
        // EEE Slider was changed. Set appropriate
        public void UIModularCharacterSetSlider(ChangeEvent<int> e, ModularPart modPart)
        {
            // https://docs.unity.cn/Manual/UIE-Events-Handling.html
            //  Debug.Log("Value: " + e.newValue);
            switch (modPart)
            {
                case ModularPart.Hair:
                    SwitchHair(e.newValue); 
                    break;
                case ModularPart.Beard:
                    SwitchBeard(e.newValue);
                    break;
                case ModularPart.Head:
                    SwitchHead(e.newValue); // Note - Female Head 23 included but mesh doesn't exist from HNG. Had to remove
                    break;
                case ModularPart.Eyebrow:
                    SwitchEyebrow(e.newValue); // ModularCustomizationManager doesn't activate. Edited code
                    break;                  
                case ModularPart.Ears:
                    SwitchEar(e.newValue); // ModularCustomizationManager doesn't activate. Edited code
                    break;                  
            }
        }

        // EEE Initializes the high value for each modular slider
        // Called from ModularCustomizationManager to make sure it exists first.
        public void InitModularCharacterSliders(ModularCustomizationManager mcm)
        {
            int i = 0; // Counter

            // Debug.Log("ModularCustomizationManager: " + mcm);
            if ( mcm != null)
            {
                for (i = 0; i < m_SectionHeadSliders.childCount; ++i)
                {
                    ...
                    if (element is ModCharSlider charSlider)
                    {
                        switch (charSlider.modularPart)
                        {
                            case ModularPart.Hair:
                                charSlider.highValue = mcm.hairModels.Count > 0 ? mcm.hairModels.Count - 1 : 0;
                                break;
                            case ModularPart.Beard:
                                charSlider.highValue = mcm.beardModels.Count > 0 ? mcm.beardModels.Count - 1 : 0;
                                break;
                            case ModularPart.Head:
                                charSlider.highValue = mcm.headModels.Count > 0 ? mcm.headModels.Count - 1 : 0;
                                break;
                            case ModularPart.Eyebrow:
                                charSlider.highValue = mcm.eyebrowModels.Count > 0 ? mcm.eyebrowModels.Count - 1 : 0;
                                break;         
                            case ModularPart.Ears:
                                charSlider.highValue = mcm.earModels.Count > 0 ? mcm.earModels.Count - 1 : 0;
                                break;                                                          
                        }
                    }
                }
